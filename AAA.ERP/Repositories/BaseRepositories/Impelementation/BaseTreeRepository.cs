...
public class BaseTreeRepository<TEntity> : IBaseTreeRepository<TEntity> where TEntity : BaseTreeEntity
{
    private ApplicationDbContext context;
    protected DbSet<TEntity> dbSet;

    public BaseTreeRepository(ApplicationDbContext _context)
    {
        context = _context;
    ...
    public async Task<List<TEntity>> GetLevel(int level = 1)
    {
        return await dbSet.Where(e => e.ParentId == null).ToListAsync();
    }

    public Task<List<TEntity>> GetChildren(Guid id, int level = 1)
    {
